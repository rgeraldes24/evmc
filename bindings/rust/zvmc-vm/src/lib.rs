...
//! Rust bindings for ZVMC (Zond Client-VM Connector API).
//!
//! Have a look at zvmc-declare to declare an ZVMC compatible VM.
...
/// ZVMC execution message structure.
#[derive(Debug)]
...
/// ZVMC transaction context structure.
pub type ExecutionTxContext = ffi::zvmc_tx_context;

/// ZVMC context structure. Exposes the ZVMC host functions, message data, and transaction context
/// to the executing VM.
pub struct ExecutionContext<'a> {
    host: &'a ffi::zvmc_host_interface,
    context: *mut ffi::zvmc_host_context,
...
impl ExecutionResult {
    ...
    pub fn failure() -> Self {
        ExecutionResult::new(StatusCode::ZVMC_FAILURE, 0, 0, None)
    }

    /// Create a revert result.
    pub fn revert(_gas_left: i64, _output: Option<&[u8]>) -> Self {
        ExecutionResult::new(StatusCode::ZVMC_REVERT, _gas_left, 0, _output)
    }

    /// Create a successful result.
    pub fn success(_gas_left: i64, _gas_refund: i64, _output: Option<&[u8]>) -> Self {
        ExecutionResult::new(StatusCode::ZVMC_SUCCESS, _gas_left, _gas_refund, _output)
    }
...
impl<'a> ExecutionContext<'a> {
    pub fn new(host: &'a ffi::zvmc_host_interface, _context: *mut ffi::zvmc_host_context) -> Self {
    ...
    pub fn call(&mut self, message: &ExecutionMessage) -> ExecutionResult {
        ...
        // zvmc_message doesn't have a release() method we could abstract it with.
        let message = ffi::zvmc_message {
            kind: message.kind(),
            flags: message.flags(),
            depth: message.depth(),
            gas: message.gas(),
        ...
        unsafe {
            ...
            (*self.host).call.unwrap()(self.context, &message as *const ffi::zvmc_message).into()
        }
    }
...
impl From<ffi::zvmc_result> for ExecutionResult {
    fn from(result: ffi::zvmc_result) -> Self {
        let ret = Self {
        ...
        if result.release.is_some() {
            unsafe {
                result.release.unwrap()(&result as *const ffi::zvmc_result);
            }
        }
...
/// Returns a pointer to a heap-allocated zvmc_result.
impl From<ExecutionResult> for *const ffi::zvmc_result {
    fn from(value: ExecutionResult) -> Self {
        let mut result: ffi::zvmc_result = value.into();
...
extern "C" fn release_heap_result(result: *const ffi::zvmc_result) {
    unsafe {
        let tmp = Box::from_raw(result as *mut ffi::zvmc_result);
        deallocate_output_data(tmp.output_data, tmp.output_size);
    }
}

/// Returns a pointer to a stack-allocated zvmc_result.
impl From<ExecutionResult> for ffi::zvmc_result {
    fn from(value: ExecutionResult) -> Self {
...
/// Callback to pass across FFI, de-allocating the optional output_data.
extern "C" fn release_stack_result(result: *const ffi::zvmc_result) {
    unsafe {
        let tmp = *result;
        deallocate_output_data(tmp.output_data, tmp.output_size);
    }
...
impl From<&ffi::zvmc_message> for ExecutionMessage {
    fn from(message: &ffi::zvmc_message) -> Self {
        ExecutionMessage {
            kind: message.kind,
            flags: message.flags,
            depth: message.depth,
...
mod tests {
    ...
    #[test]
    fn result_new() {
        let r = ExecutionResult::new(StatusCode::ZVMC_FAILURE, 420, 21, None);

        assert_eq!(r.status_code(), StatusCode::ZVMC_FAILURE);
    ...
    extern "C" fn test_result_dispose(result: *const ffi::zvmc_result) {
        unsafe {
            if !result.is_null() {
                let owned = *result;
                Vec::from_raw_parts(
    ...
    #[test]
    fn result_from_ffi() {
        let f = ffi::zvmc_result {
            status_code: StatusCode::ZVMC_SUCCESS,
            gas_left: 1337,
            gas_refund: 21,
        ...
        assert_eq!(r.status_code(), StatusCode::ZVMC_SUCCESS);
    ...
    fn result_into_heap_ffi() {
        ...
        let f: *const ffi::zvmc_result = r.into();
        assert!(!f.is_null());
        unsafe {
            assert_eq!((*f).status_code, StatusCode::ZVMC_FAILURE);
    ...
    fn message_new_with_input() {
        ...
        assert_eq!(ret.kind(), MessageKind::ZVMC_CALL);
    ...
    fn message_from_ffi() {
        ...
        let msg = ffi::zvmc_message {
            kind: MessageKind::ZVMC_CALL,
            flags: 44,
            depth: 66,
            gas: 4466,
            recipient,
    ...
    unsafe extern "C" fn get_dummy_tx_context(
        _context: *mut ffi::zvmc_host_context,
    ) -> ffi::zvmc_tx_context {
        ffi::zvmc_tx_context {
    ...
    unsafe extern "C" fn execute_call(
        ...
        ffi::zvmc_result {
            status_code: if success {
                ...
                StatusCode::ZVMC_INTERNAL_ERROR
            },
            gas_left: 2,
            gas_refund: 0,
    ...
    fn get_dummy_host_interface() -> ffi::zvmc_host_interface {
        ffi::zvmc_host_interface {
            account_exists: None,
            get_storage: None,
            set_storage: None,
            get_balance: None,
    ...
    fn test_call_empty_data() {
        ...
        assert_eq!(b.status_code(), StatusCode::ZVMC_SUCCESS);
